...
create or replace function set_order_archived_for_admin(
  input_order_id uuid,
...
declare
  order_row orders%rowtype;
begin
  select * into order_row from orders where id = input_order_id for update;
  ...
  if input_archived then
    ...
    update orders set archived_at = now() where id = input_order_id
  ...
  update orders set archived_at = null where id = input_order_id;
  return query select 'restored'::text, null::timestamp with time zone;
end;
$$;

revoke all on function set_order_archived_for_admin(uuid, boolean) from public;
revoke all on function set_order_archived_for_admin(uuid, boolean) from anon;
revoke all on function set_order_archived_for_admin(uuid, boolean) from authenticated;
grant execute on function set_order_archived_for_admin(uuid, boolean) to service_role;

create or replace function delete_order_for_admin(input_order_id uuid)
...
revoke all on function delete_order_for_admin(uuid) from public;
revoke all on function delete_order_for_admin(uuid) from anon;
revoke all on function delete_order_for_admin(uuid) from authenticated;
grant execute on function delete_order_for_admin(uuid) to service_role;
