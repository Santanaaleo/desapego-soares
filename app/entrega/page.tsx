...
export default function EntregaPage() {
  ...
  function submit(event: React.FormEvent) {
    ...
    window.open(buildWhatsAppUrl(items, form), "_blank", "noopener,noreferrer");
  }
          ...
          <form onSubmit={submit} className="grid gap-6 lg:grid-cols-[1fr_360px]">
            <div className="grid gap-5">
              ...
              <div className="rounded-md border border-neutral-200 bg-white p-5">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <div>
                    <Input required value={form.cep} onChange={(event) => updateField("cep", event.target.value)} onBlur={fetchCep} placeholder="CEP" />
                    {cepStatus ? <p className="mt-2 text-xs font-bold text-neutral-500">{cepStatus}</p> : null}
                  </div>
                  <Button type="button" variant="secondary" onClick={fetchCep} disabled={loadingCep}>
              ...
              <div className="rounded-md border border-neutral-200 bg-neutral-50 p-5">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <div className="rounded-md border border-neutral-200 bg-white p-4">
                    <p className="font-black uppercase text-neutral-950">PAC</p>
                    <p className="mt-1 text-sm font-bold text-neutral-500">Valor calculado no atendimento.</p>
                  </div>
                  <div className="rounded-md border border-neutral-200 bg-white p-4">
                    <p className="font-black uppercase text-neutral-950">SEDEX</p>
                    <p className="mt-1 text-sm font-bold text-neutral-500">Valor calculado no atendimento.</p>
                  </div>
                </div>
              </div>
